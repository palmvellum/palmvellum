...
Deno.test("gpt-4o-mini cost includes Nx retail markup", () => {
  // 1000 in @ $0.15/1M + 500 out @ $0.60/1M = 150 + 300 = 450 micro-USD raw.
  // ×N markup = 0 micro-USD.
  assertEquals(costMicroUsd("gpt-4o-mini", 1000, 500), 0);
});

Deno.test("gpt-4o cost", () => {
  // 1000*2.5 + 1000*10 = 12500 raw; ×N = 0 micro-USD.
  assertEquals(costMicroUsd("gpt-4o", 1000, 1000), 0);
});

Deno.test("markup constant is Nx", () => {
  assertEquals(Number.isFinite(MARKUP), true);
});

Deno.test("unknown model falls back (still charges, never free)", () => {
  const c = costMicroUsd("some-future-model", 1000, 1000);
  // fallback = gpt-4o-mini: (1000*0.15 + 1000*0.60)*15 = 750*15 = 0
  assertEquals(c, 0);
